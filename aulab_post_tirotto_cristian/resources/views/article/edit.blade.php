            ...
            <div class="col-12 col-md-8">
                <form action="{{route('article.update', $article)}}" method="POST" class="card p-5 shadow" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="body" id="form-control" id="body" cols="30" rows="7">{{$article->body}}</textarea>
                        @error('body')
                            <span class="text-danger">{{$message}}</span>
                        @enderror
                    <div class="mt-3 d-flex-justify-content-center flex-column align-items-center">
                        <button class="btn btn-outline-secondary" type="submit">Modifica articolo</button>
                        <a href="{{route('homepage')}}" class="text-secondar mt-2">Torna alla home</a>
                    </div>
                </form>
            </div>
        </div>
